...
async function send(text = input.value) {
  ...
  try {
    const response = await fetch(`${import.meta.env.VITE_API_BASE}/ai/chat/stream`, { method: 'POST', headers: { token: localStorage.getItem('sky_admin_token') || '', Accept: 'text/event-stream', 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: null, message: prompt, context: {} }) })
    ...
    const handleEvent = (block: string) => {
      ...
      if (event === 'meta') aiConnected.value = true
