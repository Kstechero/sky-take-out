...
import {
  cancelOrder, completeOrder, confirmOrder, createResource, deleteResource, deliverOrder,
  getCategoryList, getCategoryPage, getDishPage, getEmployeePage, getOrderDetail, getOrderPage, getOrderStatistics,
  getResourceDetail, getSetmealPage, rejectOrder, toggleResource, updateResource
} from '@/api/admin'
...
const emptyForm = () => ({
  id: null, name: '', username: '', phone: '', sex: '1', idNumber: '',
  type: 1, sort: 1, categoryId: null, price: 0, image: '', description: '', status: 0
})
const form = reactive<any>(emptyForm())

const configs: Record<string, any> = {
  order: { eyebrow: 'ORDER FLOW', desc: '查看订单详情并按照业务状态处理订单', action: '刷新订单', columns: ['订单号', '订单内容', '用户', '金额', '状态', '下单时间', '操作'] },
  ...
  coupon: { eyebrow: 'GROWTH TOOLKIT', desc: '创建活动并追踪优惠券使用效果', action: '新建优惠券', columns: ['优惠券名称', '类型', '门槛', '剩余库存', '有效期', '状态'] },
  review: { eyebrow: 'VOICE OF CUSTOMER', desc: '查看评价并使用 AI 辅助回复', action: 'AI 回复设置', columns: ['评价内容', '用户', '关联菜品', '评分', '状态', '时间'] },
  sensitive: { eyebrow: 'CONTENT SAFETY', desc: '动态维护内容安全词库', action: '新增敏感词', columns: ['敏感词', '分类', '命中次数', '状态', '更新时间', '操作'] }
}

const kind = computed(() => route.meta.kind as string || 'order')
const config = computed(() => configs[kind.value])
const supported = computed(() => ['order', 'dish', 'category', 'setmeal', 'employee'].includes(kind.value))
const title = computed(() => `${editing.value ? '修改' : '新增'}${({ dish: '菜品', category: '分类', employee: '员工' } as any)[kind.value] || '信息'}`)
...
async function refresh() {
  ...
  try {
    ...
    else if (kind.value === 'dish') res = await getDishPage(params)
    else if (kind.value === 'employee') res = await getEmployeePage(params)
    else if (kind.value === 'category') res = await getCategoryPage(params)
    else if (kind.value === 'setmeal') res = await getSetmealPage(params)
    else { rows.value = []; total.value = 0; return }
...
async function primaryAction() {
  ...
  if (!['dish', 'category', 'employee'].includes(kind.value)) return ElMessage.info(`${config.value.action}表单将在对应业务模块中实现`)
  resetForm(); editing.value = false
  if (kind.value === 'dish') await loadDishCategories()
  dialog.value = true
}

async function edit(row: any) {
  resetForm(); editing.value = true
  let data = row
  ...
  if (kind.value === 'dish') {
    ...
    flavors.value = (data.flavors || []).map((item: any) => {
      let options: string[] = []
      try { options = JSON.parse(item.value || '[]') } catch { options = [] }
...
async function save() {
  try { validate() } catch (e: any) { return ElMessage.warning(e.message) }
  saving.value = true
  try {
...
function addFlavor() { flavors.value.push({ name: '', options: [] }) }
...
function presetOptions(name: string) { return flavorPresets[name] || [] }
...
async function toggle(row: any) { await toggleResource(kind.value as any, row.id, row.status === 1 ? 0 : 1); ElMessage.success('状态已更新'); refresh() }
async function remove(row: any) { await ElMessageBox.confirm(`确定删除“${row.name}”吗？`, '删除确认', { type: 'warning' }); await deleteResource(kind.value as any, row.id); ElMessage.success('删除成功'); refresh() }
...
function changeOrderStatus(status?: number) { orderStatus.value = status; page.value = 1; refresh() }

watch(kind, () => { keyword.value = ''; page.value = 1; refresh() })
onMounted(refresh)
</script>
...
<template>
  <div class="business-page">
    ...
    <section class="card table-card">
      ...
      <div class="table-tools"><el-input v-model="keyword" :prefix-icon="Search" :placeholder="kind === 'order' ? '输入订单号搜索' : '输入关键词搜索'" clearable @keyup.enter="refresh"/><el-button @click="refresh">查询</el-button></div>
      <el-alert v-if="!supported" title="该模块后端尚未实现" type="warning" :closable="false" style="margin-bottom:14px"/>
